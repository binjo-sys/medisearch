...
function xpForLevel(l){return 25+(l-1)*35}
...
function gainRep(n){const before=state.level;state.rep+=n;state.level=1;let threshold=25;while(state.rep>=threshold){state.level++;threshold+=35}if(state.level>before)toast(`LEVEL UP! You reached level ${state.level} 🔥`)}
function action(type,arg){
 ...
 if(type==='travel'){state.location=arg;state.energy=Math.max(0,state.energy-5);toast(`You moved to ${arg}`);save();return}
